...
def get_df(
    ...
    print("Memory Usage:")
    print(
        "Total:    ", round(psutil.virtual_memory().total / 1024**3, 1), "GB"
    )
    print(
        "Available before loading :",
        round(psutil.virtual_memory().available / 1024**3, 1),
        "GB",
    )
    dm.prepare_data()
    dm.setup()
    ...
    data = data.reshape(data.shape[0], data.shape[1] // 3, 3, data.shape[2])[
        :, :, :2, :
    ]
    ...
    df = pd.DataFrame(
        -data[0].moveaxis(-1, 0).reshape(-1, 2), columns=["x", "y"]
    )
    print(df)
    df["point"] = point_names * data.shape[-1]
    df["point_id"] = point_ids * data.shape[-1]
    df["frame"] = (
        np.array(range(data.shape[-1]))[:, None]
        .repeat(len(point_names), axis=1)
        .reshape(-1)
    )
    print(df)
    return df, data
...
if __name__ == "__main__":
    ...
    slider = pn.widgets.IntSlider(
        name="Frame", start=0, end=data.shape[-1] - 1, value=0
    )
    ...
    def edgesplot(edges, name=None, canvas=None, cvsopts={}):
        canvas = ds.Canvas(**cvsopts) if canvas is None else canvas
        return tf.shade(
            canvas.line(edges, "x", "y", agg=ds.count()), name=name
        )
    ...
    def plot_points(frame=0):
        ...
        return plot_points_df_frame(df_frame)

    @pn.depends(slider.param.value)
    def plot_points_normalize(frame=0):
        df_frame = df_normalize[df_normalize["frame"] == frame]
        return plot_points_df_frame(df_frame)

    def plot_points_df_frame(df_frame, frame=0):
        # df_frame = df[df["frame"] == frame]
        ...
        graph = graphplot(
            nodes,
            connect_edges(nodes, edges),
            cat="point_id",
            canvas=cvs,
            cvsopts={"plot_height": 400, "plot_width": 400},
        )
        return graph

    accelerations = []
    for point in range(data.shape[1]):
        acceleration2d = np.linalg.norm(
            data[0, point, :, 1:] - data[0, point, :, 0:-1], axis=0
        )
